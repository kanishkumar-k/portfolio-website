...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const filePath = "data/experience.json";
    ...
    const res = await fetch(
      `https://api.github.com/repos/${GITHUB_REPO}/contents/${filePath}`,
      {
        ...
        body: JSON.stringify({
          message: "Update experience.json via API",
          content,
    ...
    return NextResponse.json(
      { error: "Failed to update experience.json via GitHub", details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
  }
}
